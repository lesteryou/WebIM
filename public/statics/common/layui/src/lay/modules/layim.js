...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    var addFriendGroup = function (data) {
        var layim_list_friend = layinMain.find(".layim-list-friend");
        var html = '<li><h5 layim-event="spread" class="layim-friendgroup'+data.id+'" data-id="'+data.id+'" lay-type="true"><i class="layui-icon"></i><span>'+data.name+'</span><em>(<cite class="layim-count"> 0</cite>)</em></h5><ul class="layui-layim-list layui-show"><li class="layim-null">该分组下已无好友了</li></ul></li>';
        layim_list_friend.append(html);
    };
